...
public class MainBody_Setting_CSEditor : Editor {
	...
	SerializedProperty Sub_ConvexProp ;
	...
	void OnEnable () {
		...
		Sub_ConvexProp = serializedObject.FindProperty ( "Sub_Convex" ) ;
...
//		}
	...
	void Set_Inspector () {
		...
		Sub_ConvexProp.boolValue = EditorGUILayout.Toggle ( "Convex" , Sub_ConvexProp.boolValue ) ;
	...
	void Create () {
		...
		if ( Collider_MeshProp.objectReferenceValue ) {
			...
			Main_MeshCollider.convex = ConvexProp.boolValue ;
		} else {
		...
		if ( Sub_Collider_MeshProp.objectReferenceValue ) {
			...
			Sub_MeshCollider.convex = Sub_ConvexProp.boolValue ;
		} else {
