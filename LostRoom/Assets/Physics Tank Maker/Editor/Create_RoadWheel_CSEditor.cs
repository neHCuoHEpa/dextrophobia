...
public class Create_RoadWheel_CSEditor : Editor {
	...
	void Set_Inspector () {
		...
		EditorGUILayout.Space () ;
		EditorGUILayout.Slider ( Reinforce_RadiusProp , 0.1f , 10.0f , "SphereCollider Radius" ) ;
	...
	void SetSusValue ( string Direction ) {
		...
		JointSpring Temp_Spring = Temp_HingeJoint.spring ;
		Temp_Spring.spring = Sus_SpringProp.floatValue ;
		Temp_Spring.damper = Sus_DamperProp.floatValue ;
		Temp_Spring.targetPosition = -Sus_TargetProp.floatValue ;
		Temp_HingeJoint.spring = Temp_Spring ;
