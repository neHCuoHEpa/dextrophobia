...
public class Static_Track_CS : MonoBehaviour {
	...
	void Initial_Settings () {
		...
		Invisible_Ang = This_Transform.localRotation ;
	...
	void Parent_Settings () {
		...
		if ( Reference_L ) {
			if ( Reference_L.GetComponent < MeshFilter > () ) {
				Reference_Radius_L = Reference_L.GetComponent < MeshFilter > ().mesh.bounds.size.y + Radius_Offset ;
		...
		if ( Reference_R ) {
			if ( Reference_R.GetComponent < MeshFilter > () ) {
				Reference_Radius_R = Reference_R.GetComponent < MeshFilter > ().mesh.bounds.size.y + Radius_Offset ;
	...
	void Anchor_Control () {
		...
		Invisible_Ang.eulerAngles = new Vector3 ( Invisible_Ang.eulerAngles.x , Ang_Y + Invert_Value , Invisible_Ang.eulerAngles.z ) ;
	}

	void Dynamic_Control () {
		// Set position.
		float Pos_X = Mathf.Lerp ( Front_Script.Invisible_Pos.x , Rear_Script.Invisible_Pos.x , 0.5f ) ;
		float Pos_Z = Mathf.Lerp ( Front_Script.Invisible_Pos.z , Rear_Script.Invisible_Pos.z , 0.5f ) ;
		Invisible_Pos = new Vector3 ( Pos_X , Invisible_Pos.y , Pos_Z ) ;
		// Set angle.
		float Ang_Y = Mathf.Rad2Deg * Mathf.Atan ( ( Front_Script.Invisible_Pos.x - Rear_Script.Invisible_Pos.x ) / ( Front_Script.Invisible_Pos.z - Rear_Script.Invisible_Pos.z) ) ;
		Invisible_Ang.eulerAngles = new Vector3 ( Invisible_Ang.eulerAngles.x , Ang_Y + Invert_Value , Invisible_Ang.eulerAngles.z ) ;
	}
	...
	void Slide_Control () {
		if ( Direction ) { // Left
			This_Transform.localPosition = Vector3.Slerp ( Invisible_Pos , Rear_Script.Invisible_Pos , Parent_Script.Rate_L ) ;
			This_Transform.localRotation = Quaternion.Slerp ( Invisible_Ang , Rear_Script.Invisible_Ang , Parent_Script.Rate_L ) ;
		} else { // Right
			This_Transform.localPosition = Vector3.Slerp ( Invisible_Pos , Rear_Script.Invisible_Pos , Parent_Script.Rate_R ) ;
			This_Transform.localRotation = Quaternion.Slerp ( Invisible_Ang , Rear_Script.Invisible_Ang , Parent_Script.Rate_R ) ;
		}
	}
