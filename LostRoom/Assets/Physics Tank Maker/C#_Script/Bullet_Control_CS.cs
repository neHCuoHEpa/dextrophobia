...
public class Bullet_Control_CS : MonoBehaviour {
	...
	void  Start () {
		...
		if ( Type == 0 ) { // AP
			Temp_BoxCollider = GetComponent < BoxCollider > () ;
			Default_Size = GetComponent < MeshFilter > ().mesh.bounds.size ;
		}
	...
	void  Hit ( GameObject Temp_Object , Vector3 Temp_Normal ) {
		Live_Flag = false ;
		if ( Type == 0 ) { // AP
			Temp_BoxCollider.size = Default_Size ;
			Temp_BoxCollider.center = Vector3.zero ;
		} else { // HE
	...
	IEnumerator Add_Explosion_Force ( Collider Temp_Target_Collider ) {
		...
		if ( Temp_Target_Collider ) {
			...
			if ( Physics.Raycast ( Temp_Ray , out Temp_RaycastHit , Explosion_Radius , layerMask ) ) {
				if ( Temp_RaycastHit.collider == Temp_Target_Collider && Temp_Target_Collider.GetComponent<Rigidbody>() ) {
					float Distance_Loss = Mathf.Pow ( ( Explosion_Radius - Temp_RaycastHit.distance ) / Explosion_Radius , 2 ) ;
					Temp_Target_Collider.GetComponent<Rigidbody>().AddForce ( Ray_Direction * Explosion_Force * Distance_Loss ) ;
