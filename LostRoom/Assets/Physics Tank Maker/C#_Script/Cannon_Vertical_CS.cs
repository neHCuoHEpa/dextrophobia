...
public class Cannon_Vertical_CS : MonoBehaviour {
	...
	void Auto_Turn () { // Calculate the angle and rate, and rotate the cannon.
		...
		if ( Target_Angle > 0.01f ) {
			Temp_Vertical = -Mathf.Lerp ( 0.0f , 1.0f , Target_Angle / ( Speed_Mag * Time.deltaTime + Buffer_Angle ) ) ;
			Rotate () ;			
	...
	void AI_Input () {
		...
		if ( AI_Script.Direct_Fire ) { // Direct Fire.
			if ( AI_Script.Detect_Flag ) { // Target is detected.
				RayCast_Count += Time.deltaTime ;
				if ( RayCast_Count > 1.0f ) {
					Cast_Ray () ; // Cast Ray and set the Fire_Flag.
					RayCast_Count = 0.0f ;
				}
	...
	void Cast_Ray () {
		...
		//Ray Temp_Ray = new Ray ( Bullet_Generator_Transform.position , AI_Script.Target_Pos - Bullet_Generator_Transform.position ) ;
		...
		if ( Physics.Raycast ( Temp_Ray , out Temp_RaycastHit , AI_Script.Visibility_Radius , layerMask ) ) {
	...
	void Fire_Process () {
		if ( Turret_Horizontal_Script.Fire_Flag && Fire_Flag ) { // Turret is ready, and Ray hits the target.
			if ( Mathf.Abs ( Target_Angle ) < AI_Script.Fire_Angle ) { // Cannon is within the angle range.
				Cannon_Count += Time.deltaTime ;
				if ( Cannon_Count > AI_Script.Fire_Count ) { // The count expired.
					if ( Cannon_Fire_Script.Reload_Flag ) { // Reloaded.
						gameObject.SendMessage ( "Fire" , SendMessageOptions.DontRequireReceiver ) ; // Send message to "Cannon_Fire".
						Cannon_Count = 0.0f ;
						Wait_Count = 0.0f ;
						Random_Offset () ;
					}
