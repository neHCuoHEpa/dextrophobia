...
public class HighRateTerminal : MonoBehaviour
{
	...
	void HandleOnDevicePicked (BluetoothDevice device)//Called when device is Picked by user
	{
		...
		//Disposing the previous instance, as we will replace it with another
		if(this.device != null) {
			this.device.Dispose();
		}
	...
	{//Manage Reading Coroutine
		...
		while (device.IsReading) {
			...
			{
				fps = charCount / dt ;
				string caption =   "Chars : " +  charCount + '\n' 
					+  "Time : " +  dt + '\n' +
					"CharPerSecond : " +  fps;
				readDataText.set(caption);
				charCount = 0;
				//dt -= 1.0/updateRate;
				dt = 0;
			}
			...
			//if (packets != null) {
				...
				//for (int i=0; i<packets.Count; i++) {
					...
					//charCount += packets.get_packet_size (i);
