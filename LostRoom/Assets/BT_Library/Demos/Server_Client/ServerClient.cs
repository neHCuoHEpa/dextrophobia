...
public class ServerClient : MonoBehaviour
{
	...
	void HandleOnClientRequest (BluetoothDevice device)
	{
		...
		//Dispose the previous reference (IF you don't need it)
		//TODO this is crazy here.. the previous device is the same as thus one (IF you picked it twice)
	...
	{
		if(this.device != null) this.device.Dispose();//Dispose the previous reference (IF you don't need it)
	...
	public void initServer ()
	{
		...
		BluetoothAdapter.startServer(UUID,100,false);
	}
	

	int NumberOfReadingDevices = 0;
	...
	{//Manage Reading Coroutine
		...
		Debug.Log("MANGE CONNECTION :::: STARTED");
		if(NumberOfReadingDevices == 0){
		//Switch to Terminal View
			InfoCanvas.SetActive (false);
			DataCanvas.SetActive (true);
		}
		NumberOfReadingDevices += 1;
